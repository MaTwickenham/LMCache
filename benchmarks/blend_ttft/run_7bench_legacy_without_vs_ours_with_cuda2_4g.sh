...
OUT_ROOT="${OUT_ROOT:-$BENCH_DIR/analysis_results/run7_four_systems_fair_cpu${CPU_BUDGET}_gpu${GPU_BUDGET}_cuda${CUDA_DEV}_${STAMP}}"
...
datasets_for() {
  ...
  case "$workload" in
    ...
    skillsbench) echo "dense_core" ;;
    swe_skillsbench) echo "swe_obs_dense" ;;
...
for workload in "${WORKLOADS[@]}"; do
  ...
  # "plain baselines":
  # - no cache / pure recompute
  # - native vLLM prefix reuse
  # Both are run under the exact same workload/config and materialized into one result.json.
  ...
  # - semantic hints disabled
  # - startup prefilled with gpu_then_cpu
  # - misses admitted with the baseline auto rule
  ...
  # "ours+with":
  # - fast connector
  # - utility-guided online execution
  # - utility-guided fragment management
  # - semantic hints enabled
  # - startup prefilled with gpu_then_cpu
  # - misses admitted with utility-guided auto placement
